...
function normalizeAndGenerateUUIDs(eventData: any): {
  normalizedData: any;
  rideRef: string | null;
} {
  ...
  if (!isUUID(normalizedData.campaign_id)) {
    console.log(
      `Invalid campaign_id: ${normalizedData.campaign_id}, generating new UUID`,
    );
    ...
    try {
      ...
      if (error.message?.includes("duplicate key") || error.code === "23505") {
        ...
        if (duplicateAttempts >= 2) {
          console.log(
            `${context} detected duplicate key after ${duplicateAttempts} attempts - skipping`,
          );
          throw error; // Let the caller handle this as a duplicate
        }
        console.log(
          `${context} detected duplicate key - attempt ${duplicateAttempts}/2`,
        );
      }
      ...
      if (attempt === RETRY_CONFIG.maxRetries) {
        console.error(
          `${context} failed after ${RETRY_CONFIG.maxRetries + 1} attempts:`,
          error,
        );
        throw error;
      }
      ...
      console.warn(
        `${context} attempt ${attempt + 1} failed, retrying in ${Math.round(delay)}ms:`,
        error.message,
      );
...
function validateEventParams(params: any): {
  isValid: boolean;
  error?: string;
} {
  ...
  if (
    !event_type ||
    !["impression", "click", "conversion", "test_sync"].includes(event_type)
  ) {
...
async function serveAd(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { normalizedData: tempData, rideRef } = normalizeAndGenerateUUIDs({
    ride_id: ride_id as string,
  });
...
async function recordBatchEvents(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const duplicates = insertResults.filter(
    ...
    if (event_type === "click") {
      try {
        await withRetry(
          () =>
            triggerDriverPayout(
              normalizedData.ride_id,
              normalizedData.campaign_id,
              normalizedData.ad_id,
            ),
          "Driver payout",
        );
      } catch (payoutError) {
