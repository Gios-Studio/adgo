...
import { Activity, Users, AlertTriangle, TrendingUp, Globe, Shield, Clock, Database } from 'lucide-react';
...
export default function TelemetryDashboard() {
  ...
  const fetchTelemetryData = async () => {
    try {
      ...
      const { data: analyticsData, error: analyticsError } = await supabase
        .rpc('get_license_analytics', { days_back: parseInt(selectedPeriod) });
  ...
  const formatEventsByDay = () => {
    ...
    return Object.entries(eventsByDay).map(([day, count]) => ({
      day,
      events: count
    })).slice(-7);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">SDK Telemetry Dashboard</h1>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data?.analytics?.active_licenses || 0}</div>
            <p className="text-xs text-muted-foreground">
              +{((data?.analytics?.active_licenses || 0) / Math.max(data?.analytics?.total_licenses || 1, 1) * 100).toFixed(1)}% active
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data?.analytics?.total_usage?.toLocaleString() || 0}</div>
            <p className="text-xs text-muted-foreground">
              Avg: {data?.analytics?.avg_usage_per_license?.toFixed(1) || 0} per license
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data?.analytics?.error_rate?.toFixed(2) || 0}%</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold capitalize">{data?.analytics?.top_region || 'Global'}</div>
            <p className="text-xs text-muted-foreground">
              Most active region
            </p>
          </CardContent>
            ...
            <table className="w-full border-collapse border border-gray-300">
              <thead>
                <tr className="bg-gray-50">
                  <th className="border border-gray-300 px-4 py-2 text-left">License Key</th>
                  ...
                  <th className="border border-gray-300 px-4 py-2 text-right">Usage</th>
                  <th className="border border-gray-300 px-4 py-2 text-right">Remaining</th>
                  <th className="border border-gray-300 px-4 py-2 text-center">Health</th>
                  <th className="border border-gray-300 px-4 py-2 text-left">Last Used</th>
                </tr>
              </thead>
              <tbody>
                {data?.license_usage_summary?.slice(0, 20).map((license, index) => (
                  <tr key={license.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="border border-gray-300 px-4 py-2 font-mono text-sm">
                      {license.license_key.substring(0, 16)}...
                    </td>
                    <td className="border border-gray-300 px-4 py-2">
                      <Badge variant={license.plan === 'enterprise' ? 'default' : license.plan === 'pro' ? 'secondary' : 'outline'}>
                        {license.plan}
                      </Badge>
                    </td>
                    <td className="border border-gray-300 px-4 py-2 capitalize">{license.region}</td>
                    <td className="border border-gray-300 px-4 py-2 text-right">
                      {license.usage_count.toLocaleString()} / {license.usage_limit.toLocaleString()}
                    </td>
                    <td className="border border-gray-300 px-4 py-2 text-right">
                      {license.remaining_calls.toLocaleString()}
                    </td>
                    <td className="border border-gray-300 px-4 py-2 text-center">
                      <div className={`inline-block w-3 h-3 rounded-full ${getHealthStatusColor(license.health_status)}`}></div>
                    </td>
                    <td className="border border-gray-300 px-4 py-2 text-sm">
                      {license.last_used ? new Date(license.last_used).toLocaleDateString() : 'Never'}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {data?.recent_events?.slice(0, 50).map((event, index) => (
              <div key={event.id} className="flex items-center justify-between p-3 border rounded-lg">
                <div className="flex items-center gap-3">
                  <Badge variant={event.event_type === 'error' ? 'destructive' : 'outline'}>
                    {event.event_type}
                  </Badge>
