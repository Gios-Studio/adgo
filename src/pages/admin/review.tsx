...
export default function AdminReviewPage() {
  ...
  const basicModerationCheck = (ad: Ad): ModerationResult => {
    const prohibitedKeywords = [
      'scam', 'fraud', 'fake', 'illegal', 'drugs', 'weapons', 'violence',
      'hate', 'discrimination', 'nsfw', 'adult', 'gambling', 'bitcoin',
      'cryptocurrency', 'get rich quick', 'miracle cure'
    ]
    ...
    return {
      ...
      reasons: flaggedKeywords.length > 0 
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'flagged':
        return <Badge variant="destructive" className="bg-orange-600">Flagged</Badge>
      default:
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {ads.filter(ad => 
                      ad.status === 'approved' && 
                      new Date(ad.created_at).toDateString() === new Date().toDateString()
                    ).length}
                  </p>
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Submitted Ads ({filteredAds.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
                          ...
                          <div>
                            ...
                            {ad.campaign && (
                              <p className="text-xs text-blue-600 mt-1">
                                Campaign: {ad.campaign.name} (${ad.campaign.budget})
                              </p>
                            )}
                          </div>
                        </div>
                                ...
                                <div className="space-y-4">
                                  ...
                                  <div className="flex gap-2">
                                    {selectedAd.status === 'pending_review' && (
                                      <>
                                        <Button
                                          onClick={() => approveAd(selectedAd.id)}
                                        ...
                                        <Button
                                          onClick={() => rejectAd(selectedAd.id, rejectionReason)}
                                          disabled={processing === selectedAd.id || !rejectionReason}
                                          variant="destructive"
                                        >
