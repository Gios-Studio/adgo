      ...
      2130: (a, b, c) => {
        c.a(a, async (a, d) => {
          try {
            ...
            let m = (0, e.createClient)(
                "https://rkonwkggxaohpmxmzmfn.supabase.co",
                process.env.SUPABASE_SERVICE_ROLE_KEY,
              ),
              n = f.z.object({
                campaign_id: f.z.string().uuid().optional(),
                partner_id: f.z.string().uuid().optional(),
                period: f.z.enum(["1h", "24h", "7d", "30d"]).default("24h"),
                format: f.z.enum(["json", "csv"]).default("json").optional(),
              });
            async function i(a, b) {
              try {
                let c = n.safeParse(a.query);
                if (!c.success)
                  return b
                    .status(400)
                    .json({
                ...
                let {
                  ...
                  i = `metrics|${d || "all"}|${f || "all"}|${e}|${h || "json"}`,
                  m = g.dY.get(i);
                if (m)
                  return (
                    b.setHeader("X-Cache", "HIT"),
                    b.setHeader("X-Cache-Key", i),
                ...
                if (
                  ...
                  return await j(a, b, e, h);
                if (d) return await k(a, b, d, h);
                if (f) return await l(a, b, f, e, h);
                return b
                  .status(400)
                  .json({ error: "missing_required_parameters" });
              } catch (a) {
                return (
              ...
              try {
                ...
                try {
                  ...
                  "24h" === c
                    ...
                    : "7d" === c &&
                      (a = "impressions_7d, clicks_7d, conversions_7d");
                  let { data: b, error: d } = await m
                  ...
                  let { data: a, error: b } = await m
                    .from("analytics_events")
                    .select("event_type, created_at")
                    .gte("created_at", d.toISOString());
                  if (b) throw b;
                  ((e =
                    a?.filter((a) => "impression" === a.event_type).length ||
                    0),
                    (f =
                      a?.filter((a) => "click" === a.event_type).length || 0),
                    (g =
                      a?.filter((a) => "conversion" === a.event_type).length ||
                      0));
                }
                let h = e > 0 ? (f / e) * 100 : 0,
                  i = f > 0 ? (g / f) * 100 : 0;
            ...
            async function k(a, b, c, d) {
              try {
                ...
                let { data: f, error: h } = await m
                  .from("campaign_ctr")
                  .select("*")
                  .eq("campaign_id", c)
                  .maybeSingle();
                if (h && "PGRST116" !== h.code) throw h;
                if (!f) {
                  let { data: d, error: e } = await m
                    .from("analytics_events")
                    .select("event_type")
                    .eq("campaign_id", c);
                  if (e) throw e;
                  let f =
                      d?.filter((a) => "impression" === a.event_type).length ||
                      0,
                    h = d?.filter((a) => "click" === a.event_type).length || 0,
                    i = f > 0 ? (h / f) * 100 : 0,
                    j = {
                      campaign_id: c,
                      impressions: f,
                      clicks: h,
                      ctr: Number(i.toFixed(2)),
                      calculated: !0,
                    };
                  return (g.dY.set(a, j, 6e4), b.status(200).json(j));
                }
                return (g.dY.set(a, f, 6e4), b.status(200).json(f));
              } catch (a) {
                return (
                  console.error("Campaign metrics error:", a),
                  b
                    .status(500)
                    .json({ error: "Failed to get campaign metrics" })
                );
              }
            }
            async function l(a, b, c, d, e) {
              try {
                ...
                if (!h || 0 === h.length) {
                  ...
                  return (g.dY.set(a, e, 6e4), b.status(200).json(e));
                }
                let j = h.map((a) => a.id),
                  { data: k, error: l } = await m
                    .from("analytics_events")
                    .select("event_type, campaign_id")
                    .in("campaign_id", j);
                if (l) throw l;
                let n =
                    k?.filter((a) => "impression" === a.event_type).length || 0,
                  o = k?.filter((a) => "click" === a.event_type).length || 0,
                  p = n > 0 ? (o / n) * 100 : 0,
                  q = {
                    partner_id: c,
                    period: d,
                ...
                return (g.dY.set(a, q, 6e4), b.status(200).json(q));
              } catch (a) {
                return (
            ...
            (f.z.object({
              period: f.z.string(),
              impressions: f.z.number().int().min(0),
              clicks: f.z.number().int().min(0),
              conversions: f.z.number().int().min(0).optional(),
              ctr: f.z.number().min(0).max(100),
              conversionRate: f.z.number().min(0).max(100).optional(),
              timestamp: f.z.string().datetime(),
            }),
              d());
          } catch (a) {
            d(a);
        ...
        c.a(a, async (a, d) => {
          try {
            ...
            async function m(a, b, c) {
              let d = await p.prepare(a, b, { srcPage: "/api/metrics/ctr" });
              if (!d) {
                ((b.statusCode = 400),
                  b.end("Bad Request"),
                  null == c.waitUntil ||
