...
`);
      var C = (t) => {
        ...
        let {
          ...
          tw = s.useMemo(
            () => T.findIndex((t) => t.toastId === x.id) || 0,
            [T, x.id],
          ),
          tN = s.useMemo(() => {
            var t;
          ...
          [tS, tB] = H.split("-"),
          tM = s.useMemo(
            () => T.reduce((t, e, a) => (a >= tw ? t : t + e.height), 0),
            [T, tw],
          ),
          tR = (() => {
        ...
        ((tj.current = s.useMemo(() => tw * _ + tM, [tw, tM])),
          ...
          }, [G, x.title, x.description, k, x.id]));
              ...
              onPointerDown: (t) => {
                tA ||
                  ...
                  "BUTTON" !== t.target.tagName &&
                    (tr(!0), (tT.current = { x: t.clientX, y: t.clientY })));
              },
              onPointerUp: () => {
                var t, e, a, r;
                if (ts || !tb) return;
